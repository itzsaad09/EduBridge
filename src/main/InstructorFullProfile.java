...
public class InstructorFullProfile extends javax.swing.JFrame {
    private String instructorId;
    private Connection connect;
    private JPanel mainContent;

    public InstructorFullProfile(String id) {
    ...
    private void setupUI() {
        setTitle("Comprehensive Instructor Profile - ID: " + instructorId);
        setSize(923, 600);
    ...
    private void loadAllData() {
        ...
        addSectionHeader("INSTRUCTOR PERSONAL INFORMATION");
        loadBasicInfo();

        // 2. Teaching Schedule Section (Based on timetable table)
        // Using instructorID to fetch schedule.
        addTableSection("TEACHING SCHEDULE / COURSES",
                "SELECT cName, day, sTime, eTime, roomNo FROM timetable WHERE instructorID = ?",
                new String[] { "Course Name", "Day", "Start Time", "End Time", "Room No" });

        // 3. Courses Details (Based on courses linked via timetable or just list
        // distinct courses)
        // We can show distinct courses they teach with more details by joining with
        // course table
        addTableSection("COURSE DETAILS",
                "SELECT DISTINCT c.coursecode, c.coursename, c.credithrs, c.session, c.academicyear, c.seats " +
                        "FROM course c " +
                        "JOIN timetable t ON c.coursecode = t.courseCode " +
                        "WHERE t.instructorID = ?",
                new String[] { "Course Code", "Course Name", "Credit Hrs", "Session", "Academic Year", "Seats" });
    }
    ...
    private void loadBasicInfo() {
        ...
        // Querying instructor table for personal details
        String query = "SELECT fName, lName, dob, cnic, phoneno, gender, department, address, email, status FROM instructor WHERE ID = ?";
        ...
        try (PreparedStatement pst = connect.prepareStatement(query)) {
            ...
            if (rs.next()) {
                ...
                addInfoRow(infoPanel, "Gender:", rs.getString("gender"));
                addInfoRow(infoPanel, "Address:", rs.getString("address"));
